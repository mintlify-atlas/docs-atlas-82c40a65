...
description: 'Give Alfred access to the web, weather data, and Hugging Face Hub statistics so he can hold informed conversations at the gala.'
---

In this section, you'll grant Alfred access to the web so he can find the latest news and global updates. You'll also give him access to weather data and Hugging Face Hub model download statistics, so he can make relevant conversation about fresh topics.

## Give Alfred access to the web

To establish Alfred's presence as a true renaissance host, he needs access to the latest news and information about the world. Let's start by creating a web search tool.
...
## Create a weather tool for scheduling fireworks

The perfect gala features fireworks under a clear sky. You need to make sure the fireworks aren't cancelled due to bad weather. Let's create a custom tool that calls an external weather API for a given location.

<Tip>
  For simplicity, this example uses a dummy weather API. If you want to use a real weather API, you could implement a weather tool that uses the OpenWeatherMap API, like in Unit 1.
</Tip>
...
## Create a Hub stats tool for influential AI builders

In attendance at the gala are the who's who of AI builders. Alfred wants to impress them by discussing their most popular models, datasets, and spaces. You'll create a tool to fetch model statistics from the Hugging Face Hub based on a username.
...
## Integrate tools with Alfred

Now that you have all the tools, let's integrate them into Alfred's agent.
...
By integrating these tools, Alfred is now equipped to handle a variety of tasks — from web searches to weather updates and model statistics — ensuring he remains the most informed and engaging host at the gala.

<Tip>
  Try implementing a tool that fetches the latest news about a specific topic. When you're done, add your custom tools to the `tools.py` file.
</Tip>
